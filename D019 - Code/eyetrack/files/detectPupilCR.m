...
function [pupil, cr1, cr2, points, edgeThresh] = detectPupilCR(img, varargin)
...
if isempty(pupilStart) || any(isnan(pupilStart)) || pupilStart(1)==0
    ...
    imgRadialPupil = Radial_Sym_Transform(img, round(radiiPupil), alphaFRST);
...
[~, inliers] = fit_ellipse_ransac(epx(:), epy(:), radiiPupil + [-20 20]);
...
if plotOn
    ...
    hold on
    a = 0:.1:2*pi;
    plot(cr1(3).*cos(a) + cr1(1), cr1(3).*sin(a)+cr1(2),'b')
    plot(cr2(3).*cos(a) + cr2(1), cr2(3).*sin(a)+cr2(2),'c')
    plot(crx, cry,'+r')
    
    % Plot ellipse
    the=linspace(0,2*pi,100);
    line(pupil(3)*cos(the)*cos(pupil(5)) - sin(pupil(5))*pupil(4)*sin(the) + pupil(1), ...
        pupil(3)*cos(the)*sin(pupil(5)) + cos(pupil(5))*pupil(4)*sin(the) + pupil(2),...
        'Color','k');
    
    plot(pupil(1), pupil(2),'+y','LineWidth',2, 'MarkerSize',10)
    if exist('epx','var')
        plot(epx, epy,'.c')
        plot(epx2, epy2,'.y')
    end
