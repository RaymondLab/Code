...
//       sortedSpikes : A 1x50 MATLAB cell array containing spike sort data.  Each cell of the array corresponds to one 
//          of 50 different "spike sort" channels. A particular cell is either a null matrix -- meaning no spike sort
//          data on that channel -- or a 1xN double array of N spike arrival times in milliseconds.  Spike times must be
//          stored chronologically, in milliseconds relative to the start of recording, and be accurate to 10us. Note
//          that the sequence of arrival times are converted into interspike intervals, which are saved as 32-bit
//          integers in 10us-tick units. If this field is an empty/null matrix, then all sorted-spike train data is
//          removed from the file. [NOTE: The number of distinct spike-sort channels was increased from 13 to 50 on
//          09sep2013.]
...
//       "sortedSpikes" => NULL or empty or a 1x50 cell matrix.  Each cell can be NULL or empty or a 1xN double array.
...
BOOL checkInput( const mxArray* pIn )
{
   ...
   // check field 'sortedSpikes':  it must be present in structure, and it can be NULL or a 1x50 cell array.  If it is 
   // non-NULL, verify that each cell is either NULL or a 1xN double array
...
void usage()
{
   ...
   printf( "      sortedSpikes : A 1x50 MATLAB cell array containing spike sort data.  Each cell of the array\n" );
   printf( "         corresponds to one of 50 possible spike sort channels, added to thedata file by analysis\n" );
...
BOOL replaceEdits( const mxArray *pIn, BOOL *bChanged )
{
   ...
   mxArray* pMxField;
   mxArray* pMxSpikeSortChannels;                                       // sorted spike trains to be put to data file
   
   *bChanged = FALSE;                                                   // haven't made any changes yet

   pMxSpikeSortChannels = mxGetField(pIn, 0, "sortedSpikes"); 
...
//    corresponding to one of the N different sorted spike train channels than can be persisted in a Maestro data file.
//
...
//          chNum -- [in] If this lies in [0..NUMSPIKESORTCH-1], then method returns TRUE if that particular cell is not 
//    empty. Else, it returns TRUE if any of the cells is not empty.
//
...
//    The method assumes that the cell array -- unless it is NULL -- has N=NUMSPIKESORTCH cells, with each cell
//    corresponding to one of the N different sorted spike train channels than can be persisted in a Maestro data file.
//    Each channel's spike times (in milliseconds) are stored in a separate set of 1 or more records. There is a unique
//    record tag for each spike sort channel: [CX_SPIKESORTREC_FIRST...CX_SPIKESORTREC_LAST]. If a give cell N in the
//    cell array is NULL, then that means there is no data for the corresponding spike train channel. Otherwise, the
//    cell will be a 1xM double array containing the M spike arrival times on that channel in chronological order,
//    relative to a starting time of 0. All times are assumed to be in milliseconds but with 10us accuracy. This is
//    important, since the arrival times are converted to **interspike intervals** in units of 10us ticks and stored as
//    integers in the file record.
//
...
//                               stored in the temporary file in the appropriate format. If NULL or empty, then there 
//                               is no spike sort data to write.
//
//    RETURNS:    TRUE if successful; FALSE if a file write error occurs.
//
BOOL writeSortedSpikes( const mxArray* pChannels )
{
   ...
   for( i=0; i<NUMSPIKESORTCH; i++ )                                    // for each supported spike sort channel:
   {
      ...
      fileRec.idTag[0] = CX_SPIKESORTREC_FIRST + i;                     //    the ID tag for this spike sort channel 
