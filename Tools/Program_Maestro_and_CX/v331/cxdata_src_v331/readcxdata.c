...
const mwSize dims[2] = {1, 1};         // to set dimensions of various fields in the output structure; the dimensions may be
...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] )
{
       ...
       (recID <= CX_XWORKACTIONREC || recID == CX_V1TGTRECORD) )
   {
      ...
      //pHdr->version = 3;
      printf( "Apple" );                                                                  //    before versioning was introduced
...
BOOL allocBuffers( BOOL bNoHeader )
{
   ...
   if( nAllOtherRecs > 0 )                                                 // alloc other bufs in a sensible way...
   {
      ...
      {                                                                    // files w/ version >=2
         ...
         if(pHdr->version >= 13)                                           //    current CXFILETGT as of v=13
         {
...
void freeBuffers()
{
   ...
   // CXFILETGT was changed in data file version 8 and again in version 13
...
//    target within the data file's CX_TGTRECORD record -- for data files with version number >= 13.
//
//    ARGS:       pTgt -- [in/out] The target definition structure.  Endianness of atomic members converted in place.
//
void endianSwapTgtDef( CXFILETGT* pTgt )
...
//
//    Analysis programs like XWork and JMWork can perform "spike sorting" on the high-resolution waveform recorded by
//    Maestro (at 25KHz) or the older Cntrlx partner app "spikesPC" (at 50KHz). The resulting "sorted-spike trains" are
//    represented by a chronological sequence of spike arrival times and are appended to the original Maestro data file
//    in records with tags CX_SPIKESORTREC_FIRST..CX_SPIKESORTREC_LAST. Thus, as many as
//
//       NUMSPIKESORTCH = CX_SPIKESORTREC_LAST - CX_SPIKESORTREC_FIRST + 1
//
//    distinct trains can be stored in the file. This used to be 13; as of 06sep13, it is 50. This increase in capacity
//    makes it possible to store all of the "units" recorded on several electrodes using the Plexon system during a
//    Maestro experiment. Plexon spike trains are added by custom analysis code, typically using EDITCXDATA.
//
...
//    of the spike (where T=0 is when recording started). All interval times are in 10us units. Here we convert all
//    event intervals back to absolute occurrence times, and convert all times to milliseconds.
//
...
//    version 8 (Maestro 2.0.0), and again as of version 13 (Maestro 2.5.0). This method handles both the current and
//    the deprecated (CXFILETGT_V7, CXFILETGT_V12) formats.
//
...
{
   int i;
   CXFILETGT* pTgt;                                                        // for data file versions 13+
   CXFILETGT_V12* pTgtV12;                                                 // for data file versions [8..12]
   CXFILETGT_V7* pTgtV7;                                                   // for data file versions [2..7]

   printf( "Data file version: %lf\n",cxData.fileHdr.version );

   if( cxData.fileHdr.version < 2 )                                        // just a consistency check
   {
      printf( "ERROR: Data file version inconsistent!  %lf\n",cxData.fileHdr.version );
      return( FALSE );
   }

   if(cxData.fileHdr.version >= 13) for(i=0; i<CX_RECORDTARGETS; i++)      // copy each tgt defn in rec to buffer:
   {
...
// === Max's Function =================================================================================================
mxArray* createInt32Double( double iValue )
{
   mxArray* pMxArr = mxCreateNumericMatrix( 1, 1, mxINT32_CLASS, mxREAL );
   if( pMxArr != NULL ) *((double*) mxGetData( pMxArr )) = iValue;
   return( pMxArr );
}
...
//    Maestro data file by analysis programs like XWork and JMWork. At a minimum, it initializes the field to a
//    1xNUMSPIKESORTCH MATLAB cell array, where all the cells are empty matrices. Then, for each sorted spike train
//    channel for which data was found in the file, the corresponding cell is set to a double array representing the
//    sequence of spike arrival times in the sorted spike train; all times are in milliseconds relative to the start of
//    recording. The spike train reported will include the effects of any manual "spike-edit"  actions -- a feature
//    introduced in JMWork.
//
...
void setHeaderOutput( mxArray* pOut )
{
   ...
   mxSetField( pMXHdr, 0, "version", createInt32Double( pHdr->version ) );
...
//    Maestro's old VSG-based FB video) and again in data file version 13 (introduction of RMVideo "movie" target). The
//    method handles both the current (CXFILETGT) and old (CXFILETGT_V7, CXFILETGT_V12) formats.
//
//    ARGS:       pOut  -- [in/out] ptr to the MATLAB structure array prepared by readcxdata().
//
//    RETURNS:    NONE.
...
void setTargetDefns(mxArray* pOut)
{
   ...
   if(cxData.fileHdr.version >= 13) for(i = 0; i < cxData.nTargets; i++)
   {
   ...
   else if(cxData.fileHdr.version >= 8) for(i = 0; i < cxData.nTargets; i++)
   {
      ...
      else if(pTgtV12->def.wType == CX_RMVTARG)      // handle target defns in file versions [8..12] properly!
      {
         ...
         // these next two fields were not present in target records for v=8..12 data files
...
//          unedited spike trains are stored in a separate sequence of 1KB records with id tags CX_SPIKESORTREC_FIRST..
//          CX_SPIKESORTREC_LAST.
...
void initializeNoisyDotsEmulator()
{
   ...
   if(ok)
   {
      ...
      if(gotRMV) rmvFP_millisec = 1.0e6 / ((double) cxData.fileHdr.d_framerate);
   }
...
BOOL shouldAdjustPatternMotionDuringVStab(int pos)
{
   ...
   if(adjust)
   {
      if(cxData.fileHdr.version >= 13)
      {
         pTgt = &(cxData.pTargets[pos]);
         if(pTgt->def.wType == CX_XYTARG && cxData.fileHdr.version < 18)
            adjust = (pTgt->def.u.xy.type != FLOWFIELD);
         else if(pTgt->def.wType == CX_RMVTARG)
            adjust = (pTgt->def.u.rmv.iType == RMV_RANDOMDOTS) && ((pTgt->def.u.rmv.iFlags & RMV_F_WRTSCREEN) != 0);
      }
      else if(cxData.fileHdr.version >= 8)
...
void prepareTgtIDs()
{
   ...
   {                                                              // can deduce the old-style IDs...
      for( i = 0; (i < cxData.nTargets) && (i < MAX_TRIALTARGS); i++ )
      {
         ...
         else if(cxData.fileHdr.version <= 12)                    // and again as of file vers = 12
