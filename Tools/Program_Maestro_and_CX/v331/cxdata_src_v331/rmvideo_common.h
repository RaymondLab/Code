...
#define RMV_CURRENTVERSION    7           // current RMVideo version number (as of 04oct2016, Maestro v3.3.1)
...
// mode index (between 1 and the total # of available modes) and FP is the current frame period in microseconds as 
...
// in microseconds. If unsuccessful (eg, mode switching is not possible), returns RMV_SIG_CMDERR. Max wait = 10 secs.
...
#define RMV_MAXFRAMELAG       50    // max # frame periods to render any single display frame.  If exceeded, RMVideo
                                    // will abort the ongoing animation sequence.
// Begin a target animation sequence.  Before a sequence begins, all targets defined by the preceding "load targets"
// command set are assumed to be turned OFF and positioned at the origin (center of screen).  This command is
// accompanied with the target motion update records for the first two frames of the animation sequence. RMVideo
// updates all targets IAW the motion update records, renders the first frame on the backbuffer, and waits for the
// monitor's vertical sync before swapping buffers.  It then begins work on the second frame.
//
// As soon as the first display frame begins, RMVideo will send an "animationStarted" signal back to Maestro.  Maestro
// waits for this signal, then starts acquiring data.  The idea here is to synchronize Maestro's data acquisition
// timeline as closely as possible to the start of a display frame.  We could do things much better with external
// triggering, but that degree of synchronization was not deemed essential.
//
// It is considered an ERROR if it takes longer than 50 actual frame periods to render any display frame of an RMVideo
// animation sequence.  If this happens when rendering the very first frame, RMVideo will abort the sequence and send
// RMV_SIG_CMDERR followed by RMV_SIG_IDLE to indicate that it has returned to the idle state.
//
...
//
// DATA:  N, {V(0) ... V(N-1)}, N, {V(0) ... V(N-1)}. Per-frame target motion vectors are supplied for
...
// RMVTGTVEC structure, in the order in which the members appear in the structure definition.  Most of these members
// are floating-pt; they are scaled by 1.0e^6 and rounded to the nearest integer for transmission to RMVideo.
//
// REPLY:  RMV_SIG_CMDERR if an error occurred, else RMV_SIG_FIRSTFRAME should be sent at the BEGINNING of the first
// animation frame.  Max wait = 50 display frames.
...
// information for display frame N prior to the beginning of frame (N-1).  THIS TIMING IS CRUCIAL; IF MAESTRO FAILS TO
// SEND THE MOTION UPDATE FOR FRAME N BEFORE FRAME (N-1) STARTS, RMVideo WILL ASSUME THERE WAS NO CHANGE FOR FRAME N!
// In this case, frame N will be a duplicate of frame N-1 -- and a "duplicateFrame" error will be sent. RMVideo will
// continue the animation, but Maestro will abort the experimental protocol upon receiving that error signal.
//
// RMVideo can only do so much during a single display frame.  If the vertical retrace occurs before RMVideo is done
// rendering the next frame in the backbuffer, a "skippedFrame" error occurs. Again, RMVideo will continue to run in
// spite of the skipped frame, but Maestro will abort the experimental protocol when a skipped frame is detected.
//
// It is considered an ERROR if it takes longer than 50 actual frame periods to render any display frame of an RMVideo
// animation sequence.  If this happens, RMVideo will abort the sequence and send RMV_SIG_SKIPFRAME (unless that signal
// or _DUPFRAME have already been sent) followed by RMV_SIG_CMDERR and RMV_SIG_IDLE to indicate that it has returned to
// the idle state.
//
// IMPORTANT:  The "duplicateFrame" or "skippedFrame" signal is sent JUST ONCE during an animation sequence.  That
// way, Maestro can use the same basic command sequence to run a trial or display targets statically in Continuous
// mode.  By design, Maestro will abort the trial immediately if an RMVideo frame drop occurs.  But in Continuous
// mode RMVideo targets are not animated frame-by-frame, so there will be lots of duplicate frames, which should just
// be ignored.
//
// DATA:  N, {V(0) ... V(N-1)} -- Target motion update vectors for the next display frame.  See description of the
// arguments to the "start animation" command.
// REPLY:  NONE.  During an animation sequence, Maestro cannot wait for command acknowledgements, and RMVideo MUST NOT
// send any.  RMVideo should only send a signal if something has gone wrong: RMV_SIG_SKIPFRAME, RMV_SIG_DUPFRAME,
// RMV_SIG_CMDERR, or RMV_SIG_QUITTING.  Maestro will check for any pending RMVideo signal prior to sending each
// RMV_CMD_UPDATEFRAME command.  RMV_SIG_SKIPFRAME and RMV_SIG_DUPFRAME are sent just once during an animation seq.
// If RMVideo aborts the animation altogether because one display frame took more than 100 frame periods to draw, then
// it sends the sequence RMV_SIG_SKIPFRAME (if not already sent), RMV_SIG_CMDERR, and RMV_SIG_IDLE upon returning to
// the idle state.
...
// DATA:  None.
// REPLY:  RMV_SIG_IDLE sent as soon as RMVideo has returned to the "idle" state.  Max wait = 10 seconds (should be a
// lot less).
...
//=====================================================================================================================
// RMVideo signals sent to Maestro.  Each "signal" is simply a single 32-bit word.  RMVideo may send a signal to
// indicate a change in state, or to notify Maestro that a serious error occurred while processing a previous command.
// The most important signal is the "animationStarted" signal, since Maestro waits for this signal to synchronize its
// timeline to the RMVideo monitor's frame rate.
//=====================================================================================================================
...
#define RMV_SIG_FIRSTFRAME    -40         // Sent at start of first frame in an animation sequence.
#define RMV_SIG_SKIPFRAME     -41         // RMVideo detected a skipped frame during animation sequence.
#define RMV_SIG_DUPFRAME      -42         // RMVideo detected a duplicate frame (b/c the next RMV_CMD_UPDATEFRAME cmd
                                          // came too late) during animation sequence.
