...
for ii = 1:length(handles)
    if strcmp(handles(ii).Name, 'VOR_GUI')
        handles(ii) = [];
        break
    end
end
...
for k = 1:min(Ns,Na)
    ...
    try
        ...
        na.Children(3).FontSize = 6;
        na.Children.Children(1).SizeData = 10;
    catch
    end
%     try
...
A.PaperSize = [30 2*tiling(1)];
A.PaperPosition = [0 0 30 2*tiling(1)];
A.PaperOrientation = 'portrait';
% Save as PDF
print(A, '-fillpage',[name '.pdf'],'-dpdf', '-r300');
% Save as Fig. Currently Disabled, Figure is hard to read and very large
%savefig(A, [name '.fig']);
