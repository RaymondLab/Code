;-----------------------------------------------------------------------------
; initialization - this is the sequencer file for integrator experiments (D006)
;
; This file is similar in most ways to the other sequencer files. The main changes
; come in the addition of several new variables (drumDir, nullEye, gain, leak)
; and the addition of several tracking functions (TRACK and TRACKOFF).
; There are also a couple of smaller changes: at the end of INIT it jumps to the
; idle loop instead of returning the chair/drum to 0 - this is because we don't
; want anything to move until we've tracked the mouse's eye. Same at the end of
...
;-----------------------------------------------------------------------------
        ...
        ; for IntTo32bit: *429496729.6,   for IntTo16bit: 6553.6
            ...
            ; variables specific to eye tracking
...
INIT:   'I  RATE   0,0             ;stop cosine on drum
            ...
            JUMP   LOOP          ;return chair to zero - specific to integrator expmts
...
KCHAIR2:    BLE    V8,vdac16(0.01),KCHAIREX ;check if we're within our epsilon
            ...
            ; DAC values might have to switch (+/-) depending on default direction of drum/chair
...
;-----------------------------------------------------------------------------
;TRACKOFF: Turn off eye position tracking
;-----------------------------------------------------------------------------
...
LIGHTON: 'L DIGOUT [.......1]
            JUMP   LOOP    ;LOOP - important for integrator expmts (no chair)
